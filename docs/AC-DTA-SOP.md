[Index](./index.md) | [Media Protection](./MP-SOP.md)
...
## 2.  REFERENCES
  a.  [ARMY Assured File Transfer Guide, July 2015](REF-A)<br>
...
### 4.4. Information System Security Officer (ISSO)
...
### 4.7. Two Person Integrity (TPI) Media Custodian  
1. Contractor providing direct support to the program.  
...
### 5.2. Media Use (RMF Control MP-2)
...
### 5.3. Role Based Schemes (RMF Control AC-2(7))  
...
### 5.4. Dual Authorization (RMF Control AC-3(2))  
...
3. The ISSO and SA shall receive notification from the CPSO when the DTA role is to be activated. The method of notification must provide an auditable trail.
   1. Upon notification the SA will ensure the DTA user is logged off all [Information System] systems.  
   2. The SA will move the DTA from the restricted user security group to the DTA security group and have the user log into a [Information System] workstation.  
   3. The SA will verify the ability to burn disc is granted.  
   4. Upon completion of the AFT by the DTA, the SA shall ensure the DTA is logged off all [Information System] systems.  
   5. The SA will move the DTA from the DTA security group to the restricted user group and have the user log into a [Information System] workstation.  
   6. The SA will verify the ability to burn disc is removed.

### 5.5. Separation of Duties (RMF Control AC-5)
1. The separation of duties for the ISSO, SA, DTA, and TPI Media Custodian under this SOP are incorporated in the [Information System] GroupPolicy roles as outlined below:
   1. The SA, ISSO, TPI Media Custodian and the DTA shall not be the same individual.
   2. The SA shall not perform security audit functions.
   3. The ISSO shall not perform SA functions.

### 5.6. Least Privilege: Authorized Access to Security Functions (RMF Control AC-6(1))  
...
### 5.7. Information Flow Enforcement (RMF Control AC-4)  
1. The flow of information for AFT is tracked throughout the [Information System] life cycle as follows:
   1. The DTA will be issued, factory fresh, accountable removable media from the TPI custodian.  
   2. The CPSO will notify the ISSO and SA per (5.d.3).  
   3. The DTA will conduct the low risk AFT and return the media to the TPI custodian and update the TPI media log per (5.g.3) and proceed with storage, dissemination, and or destruction.
...
3. The TPI media custodian shall ensure the following information is recorded in the TPI log:
   1. Date – Date of the AFT;  
   2. DTA – Name of the user conducting the AFT;  
   3. File Names – Formatted as [YYYYMMDD_XX] (i.e., 20101225_01).  
   4. Source – The system source the files are transferred. (e.g., [Information System]);  
   5. Destination – Where the files will be transferred to. (e.g., Government, ASEP).
...
### 5.8.  Role Based Security Training (RMF Control AT-3)
...
### 5.9. Protection of Information at Rest: Cryptographic Protection (RMF Control SC-28(1))  
...
3. Select ```[ADD TO ARCHIVE]``` and follow the below method to name the archive.
   1. AFT File names shall be formatted per (5.g.3.a).
   **Note:** File extensions such as .zip, .7z are assumed in AFT archived filenames and need not be recorded in the media log.
   2. Sequential AFT archives on the same date will increment in the suffix.
...
4. The DTA shall type the approved known access code in the password area under ```[ENTER PASSWORD]``` and ```[REENTER PASSWORD]``` blocks.
   1. The approved known access code used for conducting AFT is not considered an authenticator, rather an access code and shall be protected similar a GSA approved container or combination lock.
   **Note:** The access code for AFTs shall be a coordinated effort between the PSO and CPSO and may have an indefinite expiration date.
   2. The access code shall be known by all who have need-to-know and only changed when directed by the PSO.
...
6. The DTA shall test the archive by following the below steps:
   ...
   2. Within the 7-Zip File Manager, navigate to the encrypted file location.
   3. Double click on the encrypted file to open;
   4. 7-Zip should challenge by requesting the access code;
   5. Enter the code/password, and verify that it works.
...
7. The DTA shall now burn the protected file archive to the removable media issued by the TPI Media Custodian by following the below steps:
   1. Navigate to the encrypted file and ```[RIGHT]``` click to open the menu options.
   2. Select ```[SEND TO]``` and chose the DVD RW Drive.
   3. Select ```[WITH A CD/DVD PLAYER]``` and click ```[NEXT]```.
   4. Click ```[BURN TO DISC]``` on the top bar, select ```[NEXT]``` and then select ```[FINISH]``` when prompted.

8. Once the burn is complete, reinsert the disc and attempt to open the file using the 7-Zip File Manager program.
   1. Click on the small, drop down arrow on the top, right hand side of the window, select ```D:``` and open the file by entering the password.
